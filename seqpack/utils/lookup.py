...
from seqpack.wire import DecoderModule, EncoderModule
...
ENCODING_SCHEMES: dict[str, EncoderModule] = {
    short: cast(EncoderModule, importlib.import_module(f"seqpack.encoding.schemes.{name}"))
    for short, name in _ENCODING_NAMES.items()
}

DECODING_SCHEMES: dict[str, DecoderModule] = {
    short: cast(DecoderModule, importlib.import_module(f"seqpack.decoding.schemes.{name}"))
