...
def _encode_payload_map(payload_map: dict, encoding_schema_json: Optional[str]) -> dict | None:
...
def _encode_column_value(value_json: str, encoding_config_json: str) -> str | None:
    ...
    return json.dumps(EncodedEntry(encoded_value, encoding_scheme, auxiliary_info).to_wire())


_encode_column_udf = F.udf(_encode_column_value, StringType()) if _PYSPARK_AVAILABLE else None


def _byte_size(value_json: str) -> int | None:
...
def _make_decode_pandas_udf():
    ...
    def _udf(encoded_json_series: pd.Series) -> pd.Series:
        def _decode_one(v):
            ...
            entry = EncodedEntry.from_wire(parsed)
            decoded = decode_feature(entry.value, entry.scheme, entry.aux)
