...
from seqpack.utils.sizing import _ensure_list
from seqpack.utils.lookup import ENCODING_SCHEMES
from seqpack.encoding.schemes.bm_encoding import BM_MAX_COUNT
...
def _auto_can_apply(step: str, values: list, used_aux: frozenset) -> bool:
    ...
    if step == "bm":
        return t == "binary" and len(values) <= BM_MAX_COUNT
    if step in ("bp", "nbp"):
...
def _auto_measure(encoded_value, scheme: str, aux: dict) -> int:
    ...
    return EncodedEntry(encoded_value, scheme, aux).byte_size()
...
def auto_encode(values, max_depth: int = _AUTO_MAX_DEPTH, lossy: bool = True) -> tuple[Any, str, dict]:
    ...
    original_size = EncodedEntry(values).byte_size()
    ...
    def dfs(current_values, current_type: str, scheme_parts: list, aux_info: dict, used_aux: frozenset, depth: int):
        ...
        for step in candidates:
            ...
            if step in ("tbqm", "tbqp") and depth > 0:
                continue
            if step == "bm" and len(current_values) > BM_MAX_COUNT:
                continue
            try:
                new_value, new_aux = ENCODING_SCHEMES[step].encode(current_values)
            except (ValueError, OverflowError, ZeroDivisionError) as exc:
                _LOG.debug("auto_encode: skipping step %r at depth %d: %s", step, depth, exc)
                continue
